 ...
 * @Last Modified time: 2019-04-20 12:47:27
 */
...
class CreateReportDialog extends PureComponent {
    ...
    handleReportRecipientSelect = (event) => {
        //First time click on select all
        if (event.target.value.indexOf('-1') > -1) {
            this.props.initialize({ priority: this.state.priorityList, service: this.props.service, name: this.props.name, description: this.props.description, account_id: this.props.account_id, time_interval: this.props.time_interval, rule_status: this.props.rule_status, compliance: this.props.compliance })
        }
    ...
    render() {
        const { users,tagList, openDialog, serviceList, accountDataList, isProgress, variant, message, showSnackbarState, complianceList } = this.state
                ...
                <DialogContent className="modal-body">
                    ...
                    <form onSubmit={handleSubmit((values) => this.reportFormSubmit(values))}>
                        ...
                        <Grid container spacing={24}>
                            ...
                            <Grid item sm={6}>
                                <Field
                                    onChangeMethod={this.handleTagSelect}
                                    format={value => Array.isArray(value) ? value : []}
                                    className="text-field" component={renderControlMultiSelect} name="tags" type="text" label="All Tags">
                                    <MenuItem className="select-item" value={'-1'} data={'-1'}>
                                        Select All
                                       </MenuItem>
                                    {
                                        tagList.map(tag => (
                                            <MenuItem data={tag} className="select-item" key={tag} value={tag}>
                                                {tag}
                                            </MenuItem>
                                        ))}
                                    }
                        ...
                        <Grid item sm={12} className="qaud-grid mrB15">
                            <Field
                                onChangeMethod={this.handleReportRecipientSelect}
                                format={value => Array.isArray(value) ? value : []}
                                className="text-field" component={renderControlMultiSelect} name="report_recipients" type="text" label="All Report Recipient">
                                {
                                    users.map(user => (
                                        <MenuItem data={user} className="select-item" key={user} value={user}>
                                            {user}
                                        </MenuItem>
                                    ))}
