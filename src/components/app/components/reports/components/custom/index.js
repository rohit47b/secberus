 ...
 * @Last Modified time: 2019-04-20 12:50:38
 */
...
import CreateReportDialog from './CreateCustomReport'
...
class CustomScheduleReport extends PureComponent {
    ...
    fetchSchedulerReport(filterData) {
        ...
        const { pageNo, sortDirection, sortBy, search, service, selectTags } = this.state
        ...
        if (selectTags && selectTags[0] !== 'Select Service') {
            let filtered_time_interval = filter(this.state.time_interval_list, function (p) {
                return includes(selectTags, p.display_name);
            });
    ...
    timeIntervalSelectHandler = name => event => {
        ...
        this.setState({ filterProgress: true, pageNo: 0, selectTags: value, dataList: [] }, () => {
            const filterData = this.props.filterData
            this.fetchSchedulerReport(filterData)
        });
        ...
        return (
            <div className="page-wrapper page-content">
                <Grid container spacing={24}>
                    <Grid item md={5}>
                        <h3 className="mr0 main-heading">Custom Reports</h3>
                    </Grid>
                    ...
                    <div className="d-flex filter-grid">
                        ...
                        <FormControl className="multi-select">
                            ...
                            >
                                <MenuItem className="select-item default-item" key={'Select Tags'} value="Select Tags" disabled>
                                </MenuItem>
                                {
                                    tagList.map(function (tag, index) {
                                        return <MenuItem className="select-item" key={tag} value={tag} >
                                            <Checkbox checked={selectTags ? selectTags.indexOf(tag) > -1 : false} />
                                            <ListItemText className="item-text" primary={tag} />
                                        </MenuItem>
                                    })
                                }
                            </Select>
                                ...
                                {({ width, height }) => (
                                    ...
                                    /> */}
                                               ...
                                               <Column
                                                    ...
                                                    label=""
                                                    headerRenderer={headerRenderer}
                                                    cellRenderer={this.viewReportCellRenderer}
                                                    disableSort={true}
                                                    width={70}
                                                    flexGrow={9}
                                                    className="table-td"
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(CustomScheduleReport));