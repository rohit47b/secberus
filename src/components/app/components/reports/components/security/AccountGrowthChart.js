 ...
 * @Last Modified time: 2019-05-30 20:13:16
 */
import * as securityActions from 'actions/securityAction';
import { store } from 'client';
import { cloneDeep } from "lodash";
import { CustomSymbol, ResponsiveLine } from 'nivo';
...
class AccountGrowthChart extends PureComponent {
    _mounted = false
    ...
    componentDidMount() {
        ...
        if (filterData.selectAccount.id !== 'all' && filterData.selectAccount.id !== undefined) {
            this._mounted = false
            this.fetchAccountGrowthTrend(filterData)
        } else {
    ...
    fetchAccountGrowthTrend(filterData) {
        const payload = {cloud_account_id: filterData.selectAccount.id, period: this.state.period}
        this.props.actions.fetchAccountGrowthTrend(payload).
            then(result => {
                this._mounted = true
                if (typeof result !== 'string') {
                    ...
                    this.setState({ area_chart_data: [{ id: "Total Assets", data: newData }], loading: false })
                } else {
                    console.log(' Error in fetching fetchAccountGrowthTrend :- ', result);
    ...
    render() {
        ...
        return (
            ...
            <div className="nivo-line-chart-container paper-box nivo-line-chart-container-height">
                 <LabelWithHelper  content={"Risk Posture History shows the last 12 months of Risk Posture Scores for your Organization and the projected Risk Posture Score after all Remediation Plans that are in progress are completed. "} message={"Risk Posture Trend"} />
                        ...
                        <div className="select-data pdR15">
                            ...
                            <FormControl className="multi-select single-select">
                                <Select
                                    ...
                                    MenuProps={{
                                        className: 'account-growth-select'
                                    }}
                                >
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(AccountGrowthChart)) 