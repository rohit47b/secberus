...
import { yesNoIconCellRenderer } from 'TableHelper/cellRenderer';
...
class VpcFlowLogAssetList extends PureComponent {
    ...
    sort = ({ sortBy, sortDirection }) => {
        if (sortBy === 'score' || sortBy === 'volume_id') {
    ...
    anchorCellRenderer = ({...params}) => {
        let component = (
            <div>
                <a  onClick={() => history.push({pathname: '/app/assets/detail/' + params.rowData.id, state: {backUrl: '/app/assets-compute', backUrlState: {value: this.props.value}}})} href="javascript:void(0)">{params.cellData}</a>
            </div>
        );
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(VpcFlowLogAssetList))