...
class LoadBalancerAssetList extends PureComponent {
            ...
            then(result => {
                if (typeof result !== 'string') {
                    result.forEach(function (type) {
                        if (type.name === 'elb_load_balancer') {
                            assetType = type.id
                        }
                    });
    ...
    securityCellRenderer = ({ cellData }) => {
        if (cellData === undefined || cellData === null || cellData.length === 0) {
            cellData = ['no security groups']
        } else {
            cellData = cellData
        }
        return (
                            ...
                            {({ onRowsRendered, registerChild }) => (
                                ...
                                >
                                    ...
                                    <Column
                                        ...
                                        headerRenderer={this.headerRenderer}
                                        cellRenderer={this.securityCellRenderer}
                                        disableSort={false}
                                        width={200}
                                        flexGrow={4}
                                    />
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(LoadBalancerAssetList))