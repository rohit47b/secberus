...
class S3AssetList extends PureComponent {
                    ...
                    if (assetType) {
                        ...
                        this.props.actions.fetchAssets(payload).
                            then(result => {
                                if (typeof result !== 'string') {
                                    let results = []
                                    let totalMfaAssets = 0
                                    let totalLogingAssets = 0
                                    ...
                                    result.forEach(function (item) {
                                        ...
                                        if (item.mfa_delete_enabled === false) {
                                            totalMfaAssets++;
                                        }
                                        if (item.logging_enabled === false) {
                                            totalLogingAssets++;
                                        }
                                    ...
                                    this.props.setCountAssets({
                                        ...
                                        totalMfaAssets: totalMfaAssets,
                                        totalLogingAssets: totalLogingAssets,
    ...
    sort = ({ sortBy, sortDirection }) => {
        if (sortBy === 'score' || sortBy === 'bucket_name') {
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(S3AssetList))