...
class EventSubscriptionAssetList extends PureComponent {
            ...
            then(result => {
                if (typeof result !== 'string') {
                    result.forEach(function (type) {
                        if (type.name === 'rds_event_subscription') {
                            assetType = type.id
                        }
                    });
                        ...
                        this.props.actions.fetchAssets(payload).
                            then(result => {
                                if (typeof result !== 'string') {
                                    ...
                                    this.props.setCountAssets({ totalAssets: this.state.dataList.length })
                                } else {
    ...
    sort = ({ sortBy, sortDirection }) => {
        if (sortBy === 'score' || sortBy === 'event_subscription_arn') {
    ...
    headerRenderer = ({ dataKey, label, sortBy, sortDirection }) => {
        ...
        if (dataKey === 'event_subscription_arn') {
            if (sortBy === dataKey) {
                        ...
                        >
                            {({ onRowsRendered, registerChild }) => (
                                ...
                                >
                                    ...
                                    <Column
                                        dataKey="event_subscription_arn"
                                        label="Identifier"
                                    ...
                                    <Column
                                        dataKey="event_subscription_status"
                                        label="Status"
                                        headerRenderer={this.headerRenderer}
                                        cellRenderer={this.validateCellData}
                                        disableSort={false}
                                    ...
                                    <Column
                                        ...
                                        label="Source IDs"
                                        headerRenderer={this.headerRenderer}
                                        cellRenderer={this.IdsCellRenderer}
                                        disableSort={false}
                                        width={120}
                                        flexGrow={6}
                                    />
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(EventSubscriptionAssetList))