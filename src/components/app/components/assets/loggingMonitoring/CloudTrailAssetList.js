...
class CloudTrailAssetList extends PureComponent {
    ...
    anchorCellRenderer = ({...params}) => {
        let component = (
            <div>
                <a  onClick={() => history.push({pathname: '/app/assets/detail/' + params.rowData.id, state: {backUrl: '/app/assets-logging-monitoring', backUrlState: {value: this.props.value}}})} href="javascript:void(0)">{params.cellData}</a>
            </div>
        );
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(CloudTrailAssetList))