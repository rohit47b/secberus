...
class PasswordPolicyAssetList extends PureComponent {
            ...
            then(result => {
                if (typeof result !== 'string') {
                    result.forEach(function (type) {
                        if (type.name === 'iam_password_policy') {
                            assetType = type.id
                        }
                    });
                    ...
                    if (assetType) {
                        ...
                        let totalPayload = { accountId: filterData.selectAccount.id, sort_by: this.state.sortBy, sort_order: this.state.sortDirection, asset_type: assetType}
                        ...
                        this.props.actions.fetchAssets(payload).
                            then(result => {
                                this._mounted = true
                                if (typeof result !== 'string') {
                                    this.props.setCountAssets({ totalAssets: result.length })
                                } else {
                                    console.log(' Error in fetching count alerts :- ', result);
    ...
    sort = ({ sortBy, sortDirection }) => {
        if (sortBy === 'score' || sortBy === 'role_id') {
    ...
    anchorCellRenderer = ({...params}) => {
        let component = (
            <div>
                <a  onClick={() => history.push({pathname: '/app/assets/detail/' + params.rowData.id, state: {backUrl: '/app/assets-identity-management', backUrlState: {value: this.props.value}}})} href="javascript:void(0)">{params.cellData}</a>
            </div>
        );
                        ...
                        >
                            {({ onRowsRendered, registerChild }) => (
                                ...
                                >
                                    ...
                                    <Column
                                        dataKey="allow_users_to_change_password"
                                        label="Allow To Change Password"
                                        headerRenderer={this.headerRenderer}
                                    ...
                                    <Column
                                        dataKey="minimum_password_length"
                                    ...
                                    <Column
                                        dataKey="password_reuse_prevention"
                                    ...
                                    <Column
                                        dataKey="require_lowercase_characters"
                                    ...
                                    <Column
                                        dataKey="require_uppercase_characters"
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(PasswordPolicyAssetList))