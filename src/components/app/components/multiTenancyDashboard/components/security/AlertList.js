...
class AlertList extends PureComponent {
      ...
      if (this.state.isMoreRecord) {
          ...
          if ((pageNo + this.state.perPage) !== this.state.pageNo) {
              this.setState({ pageNo: this.state.pageNo + this.state.perPage, filterProgress: true }, () => {
                  const filterData = this.props.filterData
                  this.fetchAlerts(this.selectedAccounts)
              });
          }
      }
  }
  ...
  alertIdAnchorCellRenderer = ({...params}) => {
    ...
    let component = (
        <div>
            <a onClick={() => history.push({pathname: '/app/alerts/detail/'+params.rowData.id, state: { alert: params.rowData, backUrl: '/app/multi-tenancy-dashboard/home', backUrlState: {openDrawer: true, alertTitle: this.props.alertTitle} }})} href="javascript:void(0)">{alertId.toUpperCase()}</a>
        </div>
    );
